...
tkey={"url":'https://www.toysrus.co.jp/ja-jp/hot-wheels/',"end_tag":'show-more',"search_tags":['<a class="card-link'],"tag_count":1,"search_result":True,"next":True}
...
def set_key(key,row,sett):
    key["url"],row=row.split(" ",1)
    a = ""
    for s in sett:
        if s in key["url"]:
            a,row=s.split("|",1)
    stag = ""
    key["end_tag"],stag=row.split("|",2)
    while "|" in stag:
        tag = ""
